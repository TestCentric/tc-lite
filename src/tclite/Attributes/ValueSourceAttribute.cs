...
namespace TCLite.Framework
{
    ...
    public class ValueSourceAttribute : DataParamAttribute, IParameterDataSource
    {
        ...
        {
            Type sourceType = SourceType ?? parameter.Member.DeclaringType;

            if (SourceName == null)
            {
                return Reflect.Construct(sourceType) as IEnumerable
                    ?? throw new Exception($"The value source type '{sourceType}' does not implement IEnumerable.");
            }
            ...
            return GetDataSourceValue(members)
        ...
        private static Exception CreateSourceNameException()
        {
            return new Exception("The sourceName specified on a ValueSourceAttribute must refer to a non-null static field, property or method.");
        }
